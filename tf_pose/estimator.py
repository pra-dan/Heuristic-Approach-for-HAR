#POPO EDITED....
#BROUGTH IN "Posture_Confidence_dict" THAT WILL CONTAIN THE VALUES REQD FOR CALCULATING THE CONFIDENCE OF EITHER OF THE 3 POSTURES
...
def IncrementDictValue(key_plus_value):
    temp = key_plus_value
    temp = temp +1
    key_plus_value = temp
...
class POPOclass :
    def isWalking(cocoDict):
        ...
        try:
            ...
            ##print('Not There')
        try:
            torsoDict["RShoulder"] = cocoDict[CocoPart.RShoulder]
        except KeyError:
            torsoDict["RShoulder"] = (0,0)
            pass
        ...
        ##PARTS FOR STUDY DETECTION
        try:
            torsoDict["RKnee"] = cocoDict[CocoPart.RKnee]
        except KeyError:
            torsoDict["RKnee"] = (0,0)
            pass

        try:
            torsoDict["Nose"] = cocoDict[CocoPart.Nose]
        except KeyError:
            torsoDict["Nose"] = (0,0)
            pass
        
        try:
            torsoDict["LEye"] = cocoDict[CocoPart.LEye]
        except KeyError:
            torsoDict["LEye"] = (0,0)
            pass
        
        try:
            torsoDict["REye"] = cocoDict[CocoPart.REye]
        except KeyError:
            torsoDict["REye"] = (0,0)
            pass

        try:
            torsoDict["LEar"] = cocoDict[CocoPart.LEar]
        except KeyError:
            torsoDict["LEar"] = (0,0)
            pass
        
        try:
            torsoDict["REar"] = cocoDict[CocoPart.REar]
        except KeyError:
            torsoDict["REar"] = (0,0)
            pass

        try:
            torsoDict["LElbow"] = cocoDict[CocoPart.LElbow]
        except KeyError:
            torsoDict["LElbow"] = (0,0)
            pass 
        try:
            torsoDict["RElbow"] = cocoDict[CocoPart.RElbow]
        except KeyError:
            torsoDict["RElbow"] = (0,0)
            pass 
        try:
            torsoDict["LWrist"] = cocoDict[CocoPart.LWrist]
        except KeyError:
            torsoDict["LWrist"] = (0,0)
            pass  
        try:
            torsoDict["RWrist"] = cocoDict[CocoPart.RWrist]
        except KeyError:
            torsoDict["RWrist"] = (0,0)
            pass      
...
class TfPoseEstimator:
    # TODO : multi-scale
    Posture_Confidence_dict = {}
    def __init__(self, graph_path, target_size=(320, 240), tf_config=None):
        self.target_size = target_size
        #POPO EDIT
        self.Posture_Confidence_dict = { "StandingOrNot" : 0,
        "SleepingOrNot" : 0,
        "SittingOrNot" : 0,
        "Max_Stand_Sleep_Sitting" : 0   ##DENOMINATOR FOR THE CONFIDENCE CALCULATION
        }
    ...
    def Reset_Posture_Confidence_dict(self):
        for (key, value) in self.Posture_Confidence_dict.items():
            self.Posture_Confidence_dict[key] = 0
    ##END
    def __del__(self):
...
#######################################################################################################################
            ...
            #plt1.show()
            #print(self.Posture_Confidence_dict)

            #ESTIMATION OF POSTURE
            
            self.Posture_Confidence_dict["Max_Stand_Sleep_Sitting"] = IncrementDictValue(self.Posture_Confidence_dict["Max_Stand_Sleep_Sitting"])
            #self.Posture_Confidence_dict["Max_Stand_Sleep_Sitting"] = self.Posture_Confidence_dict["Max_Stand_Sleep_Sitting"] + 1
            ...
            if (max(ratio1,ratio2) > 1.81  or (left_theta < 65 and left_theta >0)) :
                #print('SITTING')SleepingOrNot
                self.Posture_Confidence_dict["SittingOrNot"] = IncrementDictValue(self.Posture_Confidence_dict["SittingOrNot"])
            elif (max(ratio1,ratio2) == 0) :
                #print('ESTIMATOR :     LYING') 
                ##self.Posture_Confidence_dict["SleepingOrNot"] = IncrementDictValue(self.Posture_Confidence_dict["SleepingOrNot"])
                pass
                #self.Posture_Confidence_dict["SleepingOrNot"] = self.Posture_Confidence_dict["SleepingOrNot"] + 1
            elif (far_part =='LAnkle' or far_part == 'RAnkle') :
                self.Posture_Confidence_dict["StandingOrNot"] = IncrementDictValue(self.Posture_Confidence_dict["StandingOrNot"])
            else :
                pass
            ###END
        ...
        #try: 
        #return npimg
        #print("cocoDict in estimator", cocoDict)               
        return npimg, self.Posture_Confidence_dict, cocoDict
        #except:
